...
enum Condition {
    Number(::acore::header::BlockNumber),
    Timestamp(u64),
}
...
impl<T: NodeInfo> LocalDataStore<T> {
    ...
    pub fn pending_transactions(&self) -> Result<Vec<PendingTransaction>, Error> {
        if let Some(val) = self
            .db
            .get(self.db_name, LOCAL_TRANSACTIONS_KEY)
    ...
    fn write_txs(&self, txs: &[TransactionEntry]) -> Result<(), Error> {
        let mut batch = ::db::DBTransaction::new();

        let local_json = ::serde_json::to_value(txs).map_err(Error::Json)?;
        let json_str = format!("{}", local_json);

        batch.put_vec(self.db_name, LOCAL_TRANSACTIONS_KEY, json_str.into_bytes());
...
mod tests {
    ...
    use std::sync::Arc;
    use acore::transaction::{Transaction, Condition, PendingTransaction};
    ...
    #[test]
    fn twice_empty() {
        let db = Arc::new(::db::MockDbRepository::init(vec!["test".into()]));

        {
            let store = super::create(db.clone(), "test", Dummy(vec![]));
    ...
    fn with_condition() {
        ...
        let transactions: Vec<_> = (0..10u64)
            .map(|nonce| {
                ...
                tx.nonce = nonce.into();
                tx.nonce_bytes = vec![nonce as u8];
...

        {
            ...
            let store = super::create(db.clone(), "test", Dummy(transactions.clone()));
            assert_eq!(store.pending_transactions().unwrap(), vec![])
        }
        {
