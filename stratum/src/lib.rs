...
impl StratumImpl {
    ...
    fn subscribe(&self, _params: Params, meta: SocketMetadata) -> RpcResult {
        ...
        Ok(match self.dispatcher.initial() {
            Some(initial) => {
                match jsonrpc_core::Value::from_str(&initial) {
                    Ok(val) => Ok(val),
                    Err(e) => {
    ...
    fn submit(&self, params: Params, meta: SocketMetadata) -> RpcResult {
        Ok(match params {
            Params::Array(vals) => {
                // first two elements are service messages (miner address & worker_id & timestamp)
                match self.dispatcher.submit(
                    ...
                    Err(submit_err) => {
                        warn!(target: "stratum","Error while submitting share: {:?}", submit_err);
                        to_value(false)
                    }
                }
            }
    ...
    fn update_peers(&self, tcp_dispatcher: &Dispatcher) {
        if let Some(job) = self.dispatcher.job() {
            if let Err(e) = self.push_work_all(job, tcp_dispatcher) {
                warn!(target: "stratum","Failed to update some of the peers: {:?}", e);
            }
        }
    }
...
mod tests {
    ...
    fn can_be_started() {
        let stratum = Stratum::start(
            &SocketAddr::from_str("127.0.0.1:19980").unwrap(),
            Arc::new(VoidManager),
            None,
        );
    ...
    impl JobDispatcher for DummyManager {
        fn initial(&self) -> Option<String> { Some(self.initial_payload.clone()) }
    ...
    fn can_push_work() {
        ...
        assert_eq!(
            "{ \"id\": 0, \"method\": \"mining.notify\", \"params\": [\"17\", \"00040008\", \
             \"100500\"] }\n",
            response
        );
    }
