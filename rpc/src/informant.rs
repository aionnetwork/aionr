...
impl fmt::Debug for RateCalculator {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result { write!(fmt, "{} req/s", self.rate()) }
}
...
impl RpcStats {
    ...
    pub fn open_session(&self) { self.active_sessions.fetch_add(1, atomic::Ordering::SeqCst); }

    /// Count session closed.
    /// Silently overflows if closing unopened session.
    pub fn close_session(&self) { self.active_sessions.fetch_sub(1, atomic::Ordering::SeqCst); }
    ...
    pub fn add_roundtrip(&self, microseconds: u32) { self.roundtrips.write().add(microseconds) }

    /// Returns number of open sessions
    pub fn sessions(&self) -> usize { self.active_sessions.load(atomic::Ordering::Relaxed) }
    ...
    pub fn approximated_roundtrip(&self) -> u32 { self.roundtrips.read().approximated_median() }
}
...
pub struct ClientNotifier {
    /// Client
    pub client: Arc<::acore::client::Client>,
}
