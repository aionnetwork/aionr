...
impl SenderReservations {
    ...
    pub fn is_empty(&self) -> bool { self.previous_ready.load(atomic::Ordering::SeqCst) }
}
...
impl Reserved {
    ...
    pub fn prospective_value(&self) -> &U256 { &self.prospective_value }
}
...
impl Future for Reserved {
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        let mut value = try_ready!(self.previous.poll().map_err(|e| {
            warn!(target:"nonce","Unexpected nonce cancellation: {}", e);
        }));
        ...
        if let Some(next) = self.next.take() {
            ...
            self.executor.spawn(
                previous
                    ...
                    .map_err(|err| error!(target:"nonce","Error dropping `Reserved`: {:?}", err)),
            );
        }
    }
}
...
impl Ready {
    ...
    pub fn matches_prospective(&self) -> bool { self.matches_prospective }
