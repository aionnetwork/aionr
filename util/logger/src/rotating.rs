...
impl RotatingLogger {
    ...
    /// Return levels
    pub fn levels(&self) -> &str { &self.levels }

    /// Return logs
    pub fn logs(&self) -> RwLockReadGuard<ArrayVec<[String; LOG_SIZE]>> { self.logs.read() }
}
...
mod test {
    ...
    fn logger() -> RotatingLogger { RotatingLogger::new("test".to_owned()) }
