...
impl From<StorageHandle> for NodeHandle {
    fn from(handle: StorageHandle) -> Self { NodeHandle::InMemory(handle) }
}

impl From<H256> for NodeHandle {
    fn from(hash: H256) -> Self { NodeHandle::Hash(hash) }
}
...
/// A `Trie` implementation using a generic `HashStore` backing database.
///
...
impl<'a> TrieDBMut<'a> {
    ...
    fn inspect<F>(
        ...
        stored: Stored,
        inspector: F,
    ) -> super::Result<Option<(Stored, bool)>>
    where
        ...
        Ok(match node {
                    ...
                    *old_val = stored_value;
                    ...
                    if let Some(child) = children[idx].take() {
                        ...
                        let (new_child, changed) =
                            self.insert_at(child, partial, value, old_val)?;
                        ...
                        let leaf = self
                            .storage
                            .alloc(Stored::New(Node::Leaf(partial.encoded(true), value)));
                ...
                if cp == existing_key.len() && cp == partial.len() {
                    ...
                    *old_val = Some(stored_value);
                    ...
                    let branch_action = self
                        .insert_inspector(branch, partial, value, old_val)?
                        .unwrap_node();
                    ...
                    let branch = self
                        .insert_inspector(branch, partial.mid(cp), value, old_val)?
                        .unwrap_node();
                    ...
                    let augmented_low = self
                        .insert_inspector(low, partial.mid(cp), value, old_val)?
                        .unwrap_node();
            ...
            Node::Extension(encoded, child_branch) => {
                ...
                if cp == 0 {
                    ...
                    // continue inserting.
                    let branch_action = self
                        .insert_inspector(Node::Branch(children, None), partial, value, old_val)?
                        .unwrap_node();
                    InsertAction::Replace(branch_action)
                } else if cp == existing_key.len() {
                    trace!(target: "trie", "complete-prefix (cp={:?}): AUGMENT-AT-END", cp);
                    ...
                    let (new_child, changed) =
                        self.insert_at(child_branch, partial.mid(cp), value, old_val)?;
    ...
    ) -> super::Result<Option<(StorageHandle, bool)>>
    {
        ...
        let opt = self.inspect(stored, move |trie, node| {
            trie.remove_inspector(node, partial, old_val)
        })?;
        ...
        match handle {
            ...
            NodeHandle::InMemory(h) => {
                match self.storage.destroy(h) {
                    ...
                    Stored::New(node) => {
                        let node_rlp =
                            node.into_rlp(|child, stream| self.commit_node(child, stream));
                        if node_rlp.len() >= 32 {
...
impl<'a> TrieMut for TrieDBMut<'a> {
    ...
    fn is_empty(&self) -> bool {
        match self.root_handle {
            NodeHandle::Hash(h) => h == BLAKE2B_NULL_RLP,
    ...
    fn remove(&mut self, key: &[u8]) -> super::Result<Option<DBValue>> {
        ...
        match self.remove_at(root_handle, key, &mut old_val)? {
            ...
            None => {
                ...
                self.root_handle = NodeHandle::Hash(BLAKE2B_NULL_RLP);
                *self.root = BLAKE2B_NULL_RLP;
            }
        }
