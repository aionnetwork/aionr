...
impl BitVecJournal {
    ...
    pub fn set(&mut self, index: usize) {
        ...
        let val = self.elems.get_mut(e_index).expect("index is out of bounds");
        *val |= 1u64 << bit_index;
...
impl Bloom {
    ...
    pub fn number_of_bits(&self) -> u64 { self.bitmap_bits }

    /// Return the number of hash functions used for `check` and `set`
    pub fn number_of_hash_functions(&self) -> u32 { self.k_num }
    ...
    pub fn saturation(&self) -> f64 { self.bitmap.saturation() }
}
