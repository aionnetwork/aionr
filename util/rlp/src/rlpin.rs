...
impl<'a> fmt::Display for Rlp<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> { write!(f, "{}", self.rlp) }
}
...
where 'a: 'view
{
    ...
    /// }
    /// ```
    pub fn as_raw(&'view self) -> &'a [u8] { self.rlp.as_raw() }

    /// Get the prototype of the RLP.
    pub fn prototype(&self) -> Prototype { self.rlp.prototype().unwrap() }

    /// Get payload info.
    pub fn payload_info(&self) -> PayloadInfo { self.rlp.payload_info().unwrap() }

    /// Get underlieing data.
    pub fn data(&'view self) -> &'a [u8] { self.rlp.data().unwrap() }
    ...
    /// }
    /// ```
    pub fn item_count(&self) -> usize { self.rlp.item_count().unwrap_or(0) }
    ...
    /// }
    /// ```
    pub fn size(&self) -> usize { self.rlp.size() }
    ...
    /// }
    /// ```
    pub fn at(&'view self, index: usize) -> Rlp<'a> { From::from(self.rlp.at(index).unwrap()) }
    ...
    /// }
    /// ```
    pub fn is_null(&self) -> bool { self.rlp.is_null() }
    ...
    /// }
    /// ```
    pub fn is_empty(&self) -> bool { self.rlp.is_empty() }
    ...
    /// }
    /// ```
    pub fn is_list(&self) -> bool { self.rlp.is_list() }
    ...
    /// }
    /// ```
    pub fn is_data(&self) -> bool { self.rlp.is_data() }
    ...
    /// }
    /// ```
    pub fn is_int(&self) -> bool { self.rlp.is_int() }
    ...
    /// }
    /// ```
    pub fn iter(&'view self) -> RlpIterator<'a, 'view> { self.into_iter() }
