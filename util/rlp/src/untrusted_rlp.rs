...
where 'a: 'view
{
    ...
    pub fn as_raw(&'view self) -> &'a [u8] { self.bytes }
    ...
    pub fn is_null(&self) -> bool { self.bytes.len() == 0 }

    pub fn is_empty(&self) -> bool {
        !self.is_null() && (self.bytes[0] == 0xc0 || self.bytes[0] == 0x80)
    }

    pub fn is_list(&self) -> bool { !self.is_null() && self.bytes[0] >= 0xc0 }

    pub fn is_data(&self) -> bool { !self.is_null() && self.bytes[0] < 0xc0 }
    ...
    pub fn iter(&'view self) -> UntrustedRlpIterator<'a, 'view> { self.into_iter() }
    ...
    pub fn decoder(&self) -> BasicDecoder { BasicDecoder::new(self.clone()) }
