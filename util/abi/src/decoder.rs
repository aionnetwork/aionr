...
) -> Result<DecodeResult, Error>
{
    match *param {
        ParamType::Address => {
            let mut slice = try!(peek(slices, offset));
            let mut address = [0u8; 32];
            address.copy_from_slice(&slice[..]);
            // if 0th isn't 0xa0, should we panic?
            //            address[0] = 0xA0;
...
mod tests {
    ...
    fn decode_address() {
        let encoded = "1111111111111111111111111111111111111111111111111111111111111111"
            .from_hex()
            .unwrap();
        let address = Token::Address([0x11u8; 32].into());
    ...
    fn decode_two_address() {
        let encoded = ("".to_owned()
            ...
            .from_hex()
            .unwrap();
        let address1 = Token::Address([0x11u8; 32].into());
        let address2 = Token::Address([0x22u8; 32].into());
    ...
    fn decode_dynamic_array_of_fixed_arrays() {
        ...
        let address1 = Token::Address([0x11u8; 32].into());
        let address2 = Token::Address([0x22u8; 32].into());
        let address3 = Token::Address([0x33u8; 32].into());
        let address4 = Token::Address([0x44u8; 32].into());
