...
impl<T: BlockEncryptor, X: PaddingProcessor> EcbEncryptor<T, X> {
    ...
    pub fn reset(&mut self) { self.block_engine.reset(); }
}
...
impl<T: BlockEncryptor, X: PaddingProcessor> CbcEncryptor<T, X> {
    ...
    pub fn reset(&mut self, iv: &[u8]) { self.block_engine.reset_with_history(&[], iv); }
}
...
impl<A: BlockEncryptor> SynchronousStreamCipher for CtrMode<A> {
    fn process(&mut self, input: &[u8], output: &mut [u8]) { self.process(input, output); }
}
...
NoPadding, PkcsPadding};
    ...
    impl CipherTest for EcbTest {
        fn get_plain<'a>(&'a self) -> &'a [u8] { &self.plain[..] }
        fn get_cipher<'a>(&'a self) -> &'a [u8] { &self.cipher[..] }
    }
    ...
    pub fn benchtest_aes_ecb_no_padding() {
        ...
        println!(
            "[benchtest_aes_ecb_no_padding] aes ecb no padding (ns/call): {}",
    ...
    pub fn benchtest_aes_cbc_pkcs_padding() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_aes_cbc_pkcs_padding] aes cbc pkcs padding (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_aes_ctr() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_aes_ctr] aes ctr (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_aes_ctr_x8() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_aes_ctr_x8] aes ctr x8 (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
}
