...
use crate::cryptoutil::{
    write_u32_be, read_u32v_be, write_u64_be, read_u64v_be, add_bytes_to_bits,
...
#[inline]
fn sha256load(v2: u32x4, v3: u32x4) -> u32x4 { u32x4(v3.3, v2.0, v2.1, v2.2) }

/// Not an intrinsic, but useful for swapping vectors.
#[inline]
fn sha256swap(v0: u32x4) -> u32x4 { u32x4(v0.2, v0.3, v0.0, v0.1) }
...
/// Not an intrinsic, but works like an unaligned load.
#[inline]
fn sha512load(v0: u64x2, v1: u64x2) -> u64x2 { u64x2(v1.1, v0.0) }
...
impl Engine512State {
    ...
    pub fn process_block(&mut self, data: &[u8]) { sha512_digest_block(&mut self.h, data); }
}
...
impl Engine256State {
    ...
    pub fn process_block(&mut self, data: &[u8]) { sha256_digest_block(&mut self.h, data); }
}
...
mod tests {
    ...
    #[cfg(feature = "benches")]
    use crate::sha2::{STATE_LEN, BLOCK_LEN, sha256_digest_block_u32, sha512_digest_block_u64};
    ...
    pub fn benchtest_sha256_block() {
        ...
        println!(
            "[benchtest_sha256_block] sha256 block (ns/call): {}",
    ...
    pub fn benchtest_sha256_10() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_sha256_10] sha256 10 (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_sha256_1k() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_sha256_1k] sha256 1k (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_sha256_64k() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_sha256_64k] sha256 64k (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_sha512_10() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_sha512_10] sha512 10 (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_sha512_1k() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_sha512_1k] sha512 1k (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
    ...
    pub fn benchtest_sha512_64k() {
        ...
        let took = time.elapsed();
        println!(
            "[benchtest_sha512_64k] sha512 64k (ns/call): {}",
            (took.as_secs() * 1000_000_000 + took.subsec_nanos() as u64) / count
        );
    }
}
