...
impl Encodable for PendingTransition {
    fn rlp_append(&self, s: &mut RlpStream) { s.append(&self.proof); }
}
...
pub trait EpochVerifier<M: ::aion_machine::Machine>: Send + Sync {
    ...
    fn verify_heavy(&self, header: &M::Header) -> Result<(), M::Error> { self.verify_light(header) }
    ...
    fn check_finality_proof(&self, _proof: &[u8]) -> Option<Vec<H256>> { None }
}

/// Special "no-op" verifier for stateless, epoch-less engines.
pub struct NoOp;

impl<M: ::aion_machine::Machine> EpochVerifier<M> for NoOp {
