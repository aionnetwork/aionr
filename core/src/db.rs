...
/// Column for header chain.
const COL_HEADER_CHAIN: &'static str = "header_chain";
...
pub trait Writable {
    ...
    fn write<T, R>(&mut self, db_name: &'static str, key: &Key<T, Target = R>, value: &T)
    where
        T: rlp::Encodable,
        R: Deref<Target = [u8]>;

    /// Deletes key from the databse.
    fn delete<T, R>(&mut self, db_name: &'static str, key: &Key<T, Target = R>)
    where
        T: rlp::Encodable,
        R: Deref<Target = [u8]>;
...
pub trait Readable {
    ...
    fn read<T, R>(&self, db_name: &'static str, key: &Key<T, Target = R>) -> Option<T>
    where
        T: rlp::Decodable,
        R: Deref<Target = [u8]>;
    ...
    fn exists<T, R>(&self, db_name: &'static str, key: &Key<T, Target = R>) -> bool
