...
public:
	...
	/// @param _padToWords if true, assume the data is padded to full words (16 bytes)
	/// @returns the number of bytes consumed in memory.
	unsigned loadFromMemory(
		unsigned _offset,
		Type const& _type = IntegerType(128),
	...
	/// Stores a 128 bit integer from stack in memory.
	...
	/// @param _padToWords if true, adds zeros to pad to multiple of 16 bytes. Array elements
	...
	/// Copies full 16 byte words in memory (regions cannot overlap), i.e. may copy more than length.
	...
	/// Helper function to shift top two values on the stack to the left.
	void leftShiftNumberOnStack2(unsigned _bits);
	...
	/// Helper function to shift top two values on the stack to the right.
	void rightShiftNumberOnStack2(unsigned _bits, bool _isSigned = false);

	/// Appends code that computes the Keccak-256 hash of the topmost stack element of 16 byte type.
	void computeHashStatic();

	/// Appends code that computes the Keccak-256 hash of the top two stack elements, 32 bytes in total.
	void computeHashStatic2();
