...
static string const g_argStaticCall = "static-call";
...
{
	if (m_args.count(g_argStaticCall))
		return;
...
Allowed options)",
	...
	desc.add_options()
		...
		(g_argStaticCall.c_str(), "Call compile method via static library.");
	...
	if (m_args.count(g_argCombinedJson))
	{
		...
		for (string const& item: boost::split(requests, m_args[g_argCombinedJson].as<string>(), boost::is_any_of(",")))
			...
			{
				printf("g_combinedJsonArgs: %s", m_args[g_argCombinedJson].as<string>().c_str());
...
void CommandLineInterface::handleCombinedJSON()
{
	...
	if (m_args.count(g_argStaticCall)) {
		const string stdinPrefix = g_stdinFileNameStr + ":";
		const size_t stdinPrefixLen = stdinPrefix.length();
		string result = dev::jsonCompactPrint(output);
		while(true) {   
			string::size_type pos(0);   
			if ((pos = result.find(stdinPrefix)) != string::npos)   
				result.replace(pos, stdinPrefixLen, "");   
			else
				break;   
		}
...
string CommandLineInterface::getResults()
{
	return m_results;
}

void CommandLineInterface::setSourceCodes(char * codes)
{
	m_sourceCodes[g_stdinFileName].append(codes).append("\n");
}
